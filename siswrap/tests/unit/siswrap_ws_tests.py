...
class TestRunHandler(object):
    ...
    def test_post_job(self, http_client, http_server, base_url, setup):
        ...
        resp = yield http_client.fetch(base_url + API_URL + "/report/run/123", method = "POST", body = json(payload))
    
        assert resp.code == 202
        payload = jsonpickle.decode(resp.body)
        assert payload["runfolder"] == "/data/testarteria1//foo"


class TestStatusHandler(object):

    @pytest.mark.gen_test
    def test_get_global_status(self, http_client, http_server, base_url, setup, monkeypatch):
        def my_get_all(self, wrapper_type): 
            return []

        monkeypatch.setattr("siswrap.siswrap.ProcessService.get_all", my_get_all)
    ...
    @pytest.mark.gen_test
    def test_get_global_filled_status(self, http_client, http_server, base_url, setup, monkeypatch): 
        def my_get_all(self, wrapper_type):
    ...
    @pytest.mark.gen_test
    def test_get_existing_status(self, http_client, http_server, base_url, setup, monkeypatch): 
        def my_get(self, pid, wrapper_type): 
            return ProcessInfo(runfolder = "foo", host = "bar", state = ProcessInfo.STATE_STARTED, proc = None, msg = None, pid = pid)

        monkeypatch.setattr("siswrap.siswrap.ProcessService.get_status", my_get)

        resp = yield http_client.fetch(base_url + API_URL + "/report/status/123") 
        assert resp.code == 200
        payload = jsonpickle.decode(resp.body)
    ...
    @pytest.mark.gen_test
    def test_get_invalid_status(self, http_client, http_server, base_url, setup, monkeypatch): 
        def my_get(self, pid, wrapper_type): 
            return ProcessInfo(runfolder = None, host = "foobar", state = ProcessInfo.STATE_NONE, proc = None, msg = None, pid = pid)
