...
class TestReportWrapper(object): 
    ...
    def test_creation(self): 
        assert isinstance(self.report.conf_svc, ConfigurationService) == True
        assert isinstance(self.report.logger, Logger) == True

    # We should be able to access the ProcessInfo's attributes 
    def test_info_attr(self): 
        assert isinstance(self.report.info, ProcessInfo) == True
        assert self.report.info.runfolder == Helper.root + "/" + Helper.runfolder 

    # And we should inherit from the base Wrapper class 
    def test_inheritance(self): 
        assert issubclass(ReportWrapper, Wrapper) == True
...
class TestWrapper(object):
    ...
    def test_creation(self, monkeypatch):
        ...
        assert isinstance(wrapper.info, ProcessInfo) == True
        assert isinstance(wrapper.conf_svc, ConfigurationService) == True
        assert isinstance(wrapper.logger, Logger) == True
    ...
    # Helper method should return return the correct wrapper object for different
    # text inputs 
    def test_new_wrapper(self): 
        qc_wrap = Wrapper.new_wrapper("qc", Helper.runfolder, Helper.conf, Helper.logger) 
        report_wrap = Wrapper.new_wrapper("report", Helper.runfolder, Helper.conf, Helper.logger) 

        assert qc_wrap.type_txt == "qc"
        assert report_wrap.type_txt == "report"

        with pytest.raises(Exception) as err: 
            unknown_wrap = Wrapper.new_wrapper("foo", Helper.runfolder, Helper.conf, Helper.logger)

    # Helper method should return the correct wrapper type in text format for different URLs
    def test_url_to_type(self):
        test_urls = ["http://arteria1:1111/v1/api/qc/run/8312", "https://arteria12:3232/v2/api/report/run/3232", 
                     "http://testweb/api/1/qc/status", "http://testtest/api/v1/report/run"]
...
class TestQCWrapper(object): 
    ...
    def test_creation(self, monkeypatch): 
        assert isinstance(self.qc.info, ProcessInfo) == True
        assert isinstance(self.qc.logger, Logger) == True
        assert isinstance(self.qc.conf_svc, ConfigurationService) == True
        ...
        monkeypatch.setattr("siswrap.siswrap.ConfigurationService.get_setting", mocked_get_setting)
        qc = QCWrapper(Helper.runfolder, Helper.conf, Helper.logger)
        assert os.path.exists(dstdir + Helper.runfolder + "/sisyphus_qc.xml") == True
    ...
    def test_inheritance(self): 
        assert issubclass(QCWrapper, Wrapper) == True
...
class TestExecString(object):
    ...
    def test_creation(self, monkeypatch):
        ...
        # 8 elements in execstring: 
        # perl, binary, runfolder, runfolderpath, mail, toadderss, sender, fromaddress
...
class TestProcessService(object):
    ...
    def test_creation(self):
        ...
        assert isinstance(ps.conf_svc, ConfigurationService) == True
        assert isinstance(ps.logger, Logger) == True
    ...
    def setup_queue(self): 
        ...
        queue = {4242: MyWrapper(4242, "qc"), 3131: MyWrapper(3131, "report"), 5353: MyWrapper(5353)}
        return queue 

    def my_poll(self, pid): 
        print "polling pid", pid
        wrapper_type = self.my_queue.get(pid).type_txt
        print "returning wrapper type", wrapper_type, "with state", self.my_queue.get(pid).info.state
    ...
    def test_polling(self, monkeypatch): 
        ...
        monkeypatch.setattr("siswrap.siswrap.ProcessService.proc_queue", self.my_queue) 
        ...
        # check that we get a state_error if the return value from poll is negative
        poll_return = -1
    ...
    # Test that we can check the status of a specific process in the process queue 
    def test_status(self, monkeypatch): 
        ...
        monkeypatch.setattr("siswrap.siswrap.ProcessService.poll_process", self.my_poll)
        ...
        # Check that if we request a pid with wrong type we get none as response
        res = ps.get_status(3131, "qc")
        assert res.pid == 3131
        assert res.state == ProcessInfo.STATE_NONE

    # Test that we can fetch the status of all the current processes in the queue 
    def test_status_all(self, monkeypatch): 
        ...
        #print "types", my_queue[3131].type_txt, my_queue[4242].type_txt, my_queue[5353].type_txt
