...
class BaseHandler(tornado.web.RequestHandler):
    ...
    # Respond with different HTTP messages depending on the return code from the process
    # Used when polling the status. 
    def write_status(self, proc_info):
        ...
        if type(proc_info) is dict: 
            ...
            if state == ProcessInfo.STATE_STARTED: 
                self.write_object(proc_info, self.HTTP_OK, "OK - still processing")
            elif state == ProcessInfo.STATE_DONE: 
                self.write_object(proc_info, self.HTTP_OK, "OK - finished processing")
            elif state == ProcessInfo.STATE_ERROR: 
                self.write_object(proc_info, self.HTTP_OK, "OK - but error occured while processing") 
            else: 
    ...
    def write_accepted(self, proc_info): 
        ...
        if state == ProcessInfo.STATE_STARTED:
            self.write_object(proc_info, self.HTTP_ACCEPTED, "Request accepted") 
        else: 
            self.write_object(proc_info, self.HTTP_ERROR, "An error occurred")

    def append_status_link(self, wrapper):
        wrapper.info.link = self.create_status_link(wrapper.type_txt, wrapper.info.pid)
...
class RunHandler(BaseHandler):
    """ Our handler for requesting the launch of a new quick report and quality control.

        Args:
            runfolder: Which runfolder to generate a report or quality control for.

        Returns:
            A status code HTTP 202 if the report generation or quality control is initialised 
            successfully, and a JSON response including a link to the status page to poll.
            An error code HTTP 500 otherwise. 

        Raises:
            RuntimeError if an empty POST body was sent in, or an unknown wrapper runner 
            was requested. 
    """
    def post(self, runfolder = "/some/runfolder"):
        try: 
            ...
            runfolder = payload.get("runfolder")
            
            # Return a new wrapper object depending on what was requested in the URL, 
            # and then ask the process service to start execution. 
            wrapper_type = Wrapper.url_to_type(url)
            wrapper = Wrapper.new_wrapper(wrapper_type, str(runfolder), SisApp.config_svc, SisApp.logger)
            result = SisApp.process_svc.run(wrapper)

            self.append_status_link(result)
            resp = {"pid": result.info.pid, "state": result.info.state, "host": result.info.host, "runfolder": result.info.runfolder, 
                    "link": result.info.link, "msg": result.info.msg}
            self.write_accepted(resp)
        except RuntimeError, err: 
            self.write_object("An error ocurred: " + str(err))

class StatusHandler(BaseHandler):
    """ Our handler for checking on the status of the report generation or quality control.
        ...
        Returns:
            JSON with fields that describe current status for requested process. 
            List of dicts with Current status of all the processes if input parameter was non-existant. 
    """
    def get(self, pid):
        try: 
            url = self.request.uri
            ...
            if pid:
                response = SisApp.process_svc.get_status(int(pid), wrapper_type)
                
                payload = {"pid": response.pid, "state": response.state, "host": response.host, "msg": response.msg}

                # If the process was found then we also want to return the runfolder 
...
class ApiHelpHandler(BaseHandler):
    def get(self):
        ...
        doc = [
            ApiHelpEntry("/qc/run/runfolder", "Run Sisyphus quality control for runfolder"),
            ApiHelpEntry("/qc/status/", "Check status of all running Sisyphus quality control jobs"),
            ApiHelpEntry("/qc/status/run_id", "Check status of a specific Sisyphus quality control job"),
            ApiHelpEntry("/report/run/runfolder", "Start Sisyphus quick report for runfolder"),
            ApiHelpEntry("/report/status/", "Check status of all Sisyphus quick reports"),
            ApiHelpEntry("/report/status/run_id", "Check status of Sisyphus quick report with run_id")
        ]
